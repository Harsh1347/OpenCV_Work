...
cap = cv2.VideoCapture('data\snakes.mp4')
...
while(cap.isOpened()):
    ...
    mask_open = cv2.morphologyEx(thresh,cv2.MORPH_OPEN,np.ones((5,5)))
    mask_close = cv2.morphologyEx(mask_open,cv2.MORPH_GRADIENT,np.ones((10,10)))
    ...
    frame = cv2.resize(frame1,(512,256)) 
    frame_1 = cv2.resize(frame2,(512,256))
