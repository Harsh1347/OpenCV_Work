...
out = cv2.VideoWriter('data//output.avi', fourcc, 15.0, (1366,768))

while(True):                                
    img = pyautogui.screenshot()
    frame = np.array(img,dtype=np.uint8)
    frame = cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)            
    cv2.rectangle(frame,(470,360),(545,400),(0,0,255))
    frame2 = frame[360:400,470:545]
    frame2 = cv2.cvtColor(frame2,cv2.COLOR_RGB2GRAY)
    _,thresh = cv2.threshold(frame2,125,255,cv2.THRESH_BINARY_INV)
    cv2.imshow('thresh',thresh)
    cv2.waitKey(1)
    if np.count_nonzero(thresh.ravel() == 255) > 170:
